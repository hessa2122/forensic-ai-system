...
def download_case_report(request, case_id):
    ...
    if is_system_admin(request.user):
        if case.assigned_to and case.assigned_to_id != request.user.id:
            notify_user(
                case.assigned_to,
                f"Admin generated a report for case {case.case_number}.",
                'report',
                report,
                title='Report generated by admin',
            )
    else:
        notify_admins(
            f"{request.user.get_username()} generated a report for case {case.case_number}.",
            'report',
            report,
